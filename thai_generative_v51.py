...
Return EXACTLY one JSON array containing every candidate integer ID once, ordered
...
def parse_ranked_candidate_ids(
    ...
    candidate_text = bracket_match.group(0) if bracket_match else text
    ...
    if not parsed:
        strategy = "regex"
        for token in re.findall(r"\d+", candidate_text):
            index = int(token)
            if 0 <= index < candidate_count and index not in parsed:
                parsed.append(index)

    complete = len(parsed) == candidate_count
    if not complete:
        missing = [index for index in range(candidate_count) if index not in parsed]
        parsed.extend(missing)

    return parsed, complete, strategy
...
class Qwen35GenerativeListwiseRanker:
    ...
    def rank(self, query_text: str, documents: list[str]) -> list[dict[str, Any]]:
        ...
        prompt = build_generative_listwise_prompt(query_text, documents)
        messages = [
            {
                "role": "user",
        ...
        self.last_parsed_count = len(order)
